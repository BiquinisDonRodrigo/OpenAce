...
2. **Gluetun port-forward**: resolves the P2P port via the Gluetun control API (`GET http://127.0.0.1:8001/v1/port_forwarded`, fallback to `/tmp/gluetun/forwarded_port`, waits ≤20s); splits API port (always `ACESTREAM_PORT=6878`, needed by Flask) from P2P port (forwarded, passed to engine via `--bind`). A background watcher polls the control API every `PORT_WATCH_INTERVAL_S` (default 45s) and restarts the engine to re-bind when ProtonVPN rotates the port. State is published to marker files under `/tmp/openace` (`vpn_mode`, `active_p2p_port`, `gluetun_p2p_port`, `engine.pid`).
3. **Engine**: `while true` restart loop (backgrounded) running `/openace/start-engine --client-console --port $API_PORT [--bind $P2P_PORT]`; re-resolves the P2P port each iteration; exponential backoff on rapid crashes (3→60s, reset after 15s uptime); output → stdout/stderr for `docker logs`.
...
Image: Python 3.10-slim; multi-stage (builder downloads AceStream engine tarball with SHA256 verification); installs ffmpeg/curl/gosu/iproute2; compiles i18n `.mo`; deep healthcheck on `/healthz` (probes engine liveness + reports P2P/VPN sync; 200 when engine up, 503 when down); non-root `openace` user. Compose variants in `docker-compose*.yaml` (simple/dev/vps/VPN; the VPN composes set `VPN_PORT_FORWARDING_PROVIDER=protonvpn` + `FIREWALL_INPUT_DOCKER_NETWORK=on`). See `DOC/` for scenarios.
