...
Al confirmar, se finaliza el setup y se redirige al login. La API de finalizacion puede devolver un `admin_token`; copialo en ese momento porque solo se muestra una vez.
...
| `POST` | `/api/setup/finalize` | Finalizar setup; puede devolver `admin_token` |
| `POST` | `/api/setup/complete` | Setup completo en una sola llamada; devuelve `admin_token` |
...
- **Cookie de sesion**: Login web normal, duracion configurable con `SESSION_DURATION_HOURS` (por defecto 24h). Se renueva automaticamente cuando queda menos del 25% de vida.
- **Bearer token**: Para integracion con reproductores y APIs externas, se crea desde el panel de admin y puede tener expiracion.
- **Token en URL**: `?token=<valor>` como parametro de consulta, util para reproductores que no soportan cabeceras HTTP.
- **HTTP Basic Auth**: Usuario y contrasena en la cabecera Authorization. La verificacion se cachea brevemente para reducir coste.

Los usuarios tambien pueden tener expiracion. Si un usuario expira, sus sesiones, tokens y Basic Auth dejan de ser validos.
