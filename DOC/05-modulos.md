...
- Streams HLS activos (procesos FFmpeg)
- Plugins cargados
...
- **Bajada / Subida**: Velocidad de transferencia en tiempo real (B/s, KB/s, MB/s)
...
Procesos FFmpeg activos para transcodificacion HLS:
- Content ID
- PID del proceso
- Estado (vivo/muerto)
- Tiempo de inactividad (se terminan automaticamente tras 60 segundos de inactividad)
...
Comprueba todos los canales de los plugins cargados de forma secuencial. Se puede filtrar por:
...
Una vez aceptado, aparece la opcion "Revocar consentimiento" al pie de la pagina. Al revocar, se pierde el acceso a la aplicacion hasta una nueva aceptacion.
...
- **Refrescar**: Fuerza una actualizacion inmediata de la lista M3U
...
**Crear usuario**: Boton "+ Nuevo usuario" con campos para nombre, contrasena (minimo 8 caracteres) y rol.

**Editar usuario**: Permite cambiar nombre, rol y contrasena (dejar vacia para no cambiarla).
...
- [Despliegue en VPS](03-despliegue-vps.md) para exponer el servicio a internet
