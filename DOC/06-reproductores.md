...
- **MPEG-TS**: `http://<host>:8888/<plugin>/mpegts.m3u` — Streaming directo sin transcodificacion
- **HLS**: `http://<host>:8888/<plugin>/hls.m3u` — Transcodificado a HLS bajo demanda via FFmpeg
...
| Uso de CPU en servidor | Ninguno (proxy directo) | Alto (FFmpeg transcodifica) |
...
- Cada stream MPEG-TS activo consume ancho de banda pero practicamente nada de CPU
- Cada stream HLS activo lanza un proceso FFmpeg que consume CPU significativa
- Los procesos FFmpeg inactivos se terminan automaticamente tras 60 segundos sin peticiones
- Un servidor modesto (2 vCPU, 2 GB RAM) puede manejar 3-5 streams HLS simultaneos o decenas de streams MPEG-TS
