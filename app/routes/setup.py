...
def _json_for_script(obj):
    ...
    Escapes </ and <!-- so user-controlled strings cannot break out of
    the script context (stored XSS defense).
    """
    ...
    for i, (key, label, url) in enumerate(steps):
        if i > 0:
            ...
            parts.append('<span class="connector '
                         ...
                         + '"></span>')
        if viewing_step == "summary" and key == "summary":
            parts.append('<span class="step step-current" aria-current="step">'
                         '<span class="dot"></span>' + label + '</span>')
        elif viewing_step == "summary" or i < viewing_idx:
            ...
            parts.append('<span class="step step-pending">'
                         '<span class="dot"></span>' + label + '</span>')
    return '<nav class="progress-bar" aria-label="Progreso de configuración">' + ''.join(parts) + '</nav>'
...
def _render_page(title, step_key, body, script=""):
    ...
    return render_page(
        title="OpenAce · " + _html_unescape(title),
        ...
        container_class="",
        robots_noindex=True,
        description="Asistente de configuración inicial de OpenAce",
    )


def _with_csrf(html):
    from app.ui.base import csrf_input
    marker = '<form method="POST" action="/setup/eula">\n'
    return html.replace(marker, marker + csrf_input() + '\n', 1)
...
_STEP1_BODY = (
    '<h1 id="setup-title">Configuraci&oacute;n inicial</h1>\n'
    ...
    '    aceptaci&oacute;n se formaliza marcando la casilla de consentimiento\n'
    '    indicada al pie de este documento y queda registrada con marca\n'
    '    temporal, direcci&oacute;n IP de origen y un marcador criptogr&aacute;fico\n'
    '    asociado a dicha acci&oacute;n.</p>\n'
    ...
    '      <li>Hash SHA-256 de un marcador t&eacute;cnico de aceptaci&oacute;n\n'
    '      por checkbox.</li>\n'
    ...
    '    <input type="checkbox" id="accept-cb" name="accept" value="1">\n'
...
_STEP2_BODY = (
    ...
    'Ya hay usuarios configurados. Puedes continuar con estos usuarios o a&ntilde;adir usuarios adicionales.</p>\n'
    '  </div>\n'
    '</div>\n'
    '<div id="admin-section" class="section">\n'
    '  <h2 class="section-title">Cuenta de administrador</h2>\n'
    ...
    '</div>\n'
    '<div class="section">\n'
    '  <h2 class="section-title">Usuarios adicionales (opcional)</h2>\n'
    ...
    '  <button type="submit" class="btn btn-primary" id="next-btn">Siguiente &rarr;</button>\n'
    '</div>\n'
    '</form>'
)
...
if(hasExisting){
  ...
  document.getElementById('next-btn').textContent='Continuar con usuarios existentes \u2192';
}
...
function renderUsers(){
  ...
  list.innerHTML=additionalUsers.map(function(u,i){
    return'<div class="item-row"><div class="item-info"><strong>'+esc(u.username)+'</strong> '+roleBadge(u.role)+'</div>'+
      '<button type="button" class="btn btn-sm btn-danger" onclick="removeUser('+i+')">Eliminar</button></div>'
  }).join('')}
...
document.getElementById('users-form').onsubmit=function(ev){
...
function submitData(body){
  ...
  fetchJSON('/setup/users',{method:'POST',body:body},12000)
  .then(function(d){
    if(d.ok){location.href=d.next}
    else{showMsg(d.error||'Error',true);btn.disabled=false}})
  .catch(function(e){showMsg(e.message||'Error de conexión',true);btn.disabled=false})}
"""
...
_STEP3_BODY = (
    ...
    '  <div class="section">\n'
    '    <h2 class="section-title">Plugins existentes</h2>\n'
    '    <div id="existing-list"></div>\n'
    '  </div>\n'
    '</div>\n'
    '<div class="section">\n'
    '  <h2 class="section-title">A&ntilde;adir plugin</h2>\n'
    '  <div class="input-group">\n'
    ...
    '    </div>\n'
    '    <div class="input-group" style="flex:1">\n'
    '      <span aria-hidden="true" style="visibility:hidden">_</span>\n'
    ...
    '</div>\n'
    '<div class="section">\n'
    '  <h2 class="section-title">Plugins a crear</h2>\n'
...
function submitPlugins(skip){
  ...
  btn.disabled=true;showMsg('');
  fetchJSON('/setup/plugins',{method:'POST',body:body},12000)
  .then(function(d){
    if(d.ok){location.href=d.next}
    else{showMsg(d.error||'Error',true);btn.disabled=false}})
  .catch(function(e){showMsg(e.message||'Error de conexión',true);btn.disabled=false})}
...
def setup_eula_page():
    if not setup_store.is_setup_required():
        return redirect("/")
    from app.ui.base import csrf_input as _csrf_input_fn
    html_body = _STEP1_BODY.replace("{csrf_input}", _csrf_input_fn())
    html = _render_page("Configuraci&oacute;n inicial", "eula",
                        html_body, _STEP1_SCRIPT)
...
def setup_users_page():
    ...
    data_js = ""
    if existing:
        data_js = "var _existing=" + _json_for_script(existing) + ";\n"
...
def setup_plugins_page():
    ...
    if existing:
        ...
        data_js = "var _existingPlugins=" + _json_for_script(safe) + ";\n"
...
def setup_summary_page():
    ...
    ip = _client_ip()
    eula_st = eula_store.status()
    users = auth_store.get_all_users()
    plugins = plugin_store.get_all()

    body = '<h1 id="setup-title">Configuraci&oacute;n inicial</h1>\n'
    ...
    if users:
        body += '<table><caption class="sr-only">Usuarios configurados</caption><thead><tr><th scope="col">Usuario</th><th scope="col">Rol</th><th scope="col">Estado</th></tr></thead><tbody>\n'
        for u in users:
    ...
    if plugins:
        body += '<table><caption class="sr-only">Plugins configurados</caption><thead><tr><th scope="col">Nombre</th><th scope="col">URL</th><th scope="col">Refresco</th><th scope="col">Estado</th></tr></thead><tbody>\n'
        for p in plugins:
...
def setup_eula_post():
    if not setup_store.is_setup_required():
        return redirect("/")
    # Verify the checkbox was actually ticked. Previously this endpoint accepted
    # any POST without inspecting the form, making the client-side checkbox
    # purely cosmetic (security bug).
    accepted_flag = request.form.get("accept") or request.form.get("accept-cb") or ""
    if accepted_flag not in ("1", "true", "on", "yes"):
        return ("Debe marcar la casilla de aceptación del EULA.", 400)
    ip = _client_ip()
    ua = request.headers.get("User-Agent", "")
    result = eula_store.accept(ip, ua, "1.0", via_checkbox=True)
    if result is None:
        return ("No se pudo registrar el consentimiento.", 500)
    if setup_store.get_current_step() is None:
...
def setup_users_post():
    ...
    if not _step_reached(setup_store.get_current_step(), "eula"):
        return jsonify({"ok": False, "error": "Debes aceptar el EULA primero"}), 409
    ...
    resp = make_response(jsonify({"ok": True, "next": "/setup/plugins"}))
    duration = 24
...
def setup_plugins_post():
    if not setup_store.is_setup_required():
        return jsonify({"ok": False, "error": "Setup ya completado"}), 409
    if not _step_reached(setup_store.get_current_step(), "users"):
        return jsonify({"ok": False, "error": "Debes configurar los usuarios primero"}), 409
...
def setup_summary_post():
    ...
    result, status = setup_store.complete_setup_atomic(_finalize)
    if result is None:
        return redirect("/")
    return result
...
def api_eula_step():
    ...
    ip = _client_ip()
    ua = request.headers.get("User-Agent", "")
    eula_store.accept(ip, ua, "1.0", via_checkbox=True)
    if setup_store.get_current_step() is None:
        setup_store.set_state("setup_started_at",
                              datetime.now(timezone.utc).isoformat())
    setup_store.set_state("setup_step", "eula")
...
def api_setup_complete():
    ...
    # Pre-validate all plugins BEFORE any mutation to avoid partial commits.
    for p in plugins_data:
        display_name = (p.get("display_name") or "").strip()
        if not display_name:
            continue
